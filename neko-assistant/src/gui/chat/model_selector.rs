...
impl ModelSelector {
    ...
    pub fn switch_model(
        ...
        // Allow users to paste or type the label (eg. "gemma3n:e2b (4.5B)")
        // by stripping any trailing parenthesized suffix so we normalize to
        // the actual model id (eg. "gemma3n:e2b").
        let normalized = model_id.trim();
        let normalized = if let Some(idx) = normalized.find('(') {
            // cut off the parenthesis and whitespace before it
            normalized[..idx].trim()
        } else {
            normalized
        };
        ...
        // selector-only mode: selection state will be synced below
