...
pub struct ChatView {
    ...
    conversation_manager: Arc<Mutex<chat_history::ConversationManager>>,
...
impl ChatView {
    pub fn new(window: &mut gpui::Window, cx: &mut gpui::Context<Self>, repo_root: PathBuf, plugins: Vec<PluginEntry>) -> Self {
        // 設定を読み込み
        ...
        let message_handler = Arc::new(MessageHandler::new(
            conversation_arc.clone(),
            conversation_manager.clone(),
            ui_tx.clone(),
            use_langchain,
            config.ollama_base_url.clone(),
            config.default_model.clone(),
            None, // MCP manager（後でmain.rsで初期化）
        ));

        // ConversationActionsを初期化
        let conversation_actions = ConversationActions::new(
            conversation_arc.clone(),
            conversation_manager.clone(),
        );
    ...
    /// 新規会話を作成して切り替え
    fn create_new_conversation(&mut self, _: &ClickEvent, window: &mut gpui::Window, cx: &mut gpui::Context<Self>) {
    ...
    /// 指定IDの会話に切り替え
    fn switch_conversation(&mut self, conversation_id: &str, window: &mut gpui::Window, cx: &mut gpui::Context<Self>) {
    ...
    fn delete_conversation(&mut self, conversation_id: &str, cx: &mut gpui::Context<Self>) {
        // 現在の会話IDを取得
        let current_id = if let Ok(conv) = self.conversation.try_lock() {
            conv.id.clone()
        } else {
            return;
        };

        if let Err(e) = self.conversation_actions.delete_conversation(conversation_id, &current_id) {
...
impl gpui::Render for ChatView {
    fn render(&mut self, _window: &mut gpui::Window, cx: &mut gpui::Context<Self>) -> impl IntoElement {
        ...
        let toolbar = div().h_flex().gap_2().p_2()
            .child(
                Button::new(gpui::SharedString::from("open_plugins"))
                    ...
                    .on_click(move |_, _win, app_cx| {
                        ...
                        let _ = app_cx.open_window(WindowOptions::default(), move |window, cx| {
                            let view = cx.new(|_| crate::gui::PluginListView::new(&repo_clone, plugins_clone.clone()));
                            ...
                            MessageType::User => {
                                ...
                                div()
                                    ...
                                    .child(
                                        div()
                                            .max_w(px(600.0))
                                            .child(ChatBubble::new(msg.content.clone(), msg_type).render())
                                    )
                            }
                            _ => {
                                // それ以外は左寄せ
        ...
        // 現在の会話IDを取得
        let current_id = if let Ok(current_conv) = self.conversation.try_lock() {
            current_conv.id.clone()
        } else {
            String::new() // ロック取得失敗時はデフォルト値
        };
