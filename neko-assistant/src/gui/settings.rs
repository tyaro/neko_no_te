    ...
    fn render(&mut self, _window: &mut gpui::Window, cx: &mut gpui::Context<Self>) -> impl IntoElement {
        ...
        content = content.child(
            div().v_flex().gap_2().child(
                div().child("Ollama Base URL:")
            ).child(
                Input::new(&self.ollama_url_input)
            )
        );
        ...
        content = content.child(
            div().v_flex().gap_2().child(
                div().child("Max History Messages:")
            ).child(
                Input::new(&self.max_history_input)
            )
        );
        ...
        let checkbox_text = if is_checked { "[✓] Use LangChain (experimental)" } else { "[ ] Use LangChain (experimental)" };
                ...
                Button::new("save_settings")
                    ...
                    .on_click(move |_, _win, cx| {
                        ...
                        let max_history = match max_history_str.parse::<usize>() {
                            Ok(n) if n > 0 => n,
                            _ => {
                                *status_msg.borrow_mut() = Some("Error: Max history must be a positive number".to_string());
                                return;
                            }
                        };
                        ...
                        match config.save() {
                            Ok(_) => {
                                *status_msg.borrow_mut() = Some("Settings saved successfully!".to_string());
                            }
                            Err(e) => {
